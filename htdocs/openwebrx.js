                ...
                switch (json.type) {
                    ...
                    case "profiles":
                        ...
                        if (window._pendingHashParams && window._pendingHashParams.profile) {
                            ...
                            // Try exact match first, then partial match (profile name without sdr_id| prefix)
                            var matchedProfile = null;
                            if (availableProfiles.indexOf(targetProfile) !== -1) {
                                matchedProfile = targetProfile;
                            } else {
                                // Try partial match: "meteor_m" should match "rtlsdr|meteor_m"
                                for (var pi = 0; pi < availableProfiles.length; pi++) {
                                    var parts = availableProfiles[pi].split('|');
                                    if (parts.length > 1 && parts[1] === targetProfile) {
                                        matchedProfile = availableProfiles[pi];
                                        break;
                                    }
                                }
                            }
